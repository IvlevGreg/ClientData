import createTableRow from './modules/create-elements.js'
import createContact from './modules/create-contact.js'
...
function processingPromice() {
  ...
  btnApproveDelete.addEventListener('click', () => {
      ...
      const response = fetch(`http://localhost:3000/api/clients/${obj.id}`,{
      method: 'DELETE'
      });
      response.then(
        result => {
