...
const classesTableButtonModify = [
  'btn',
  'table__btn',
  'table__btn_modify',
  'js-btn-modify',
];
const classesTableButtonDelete = [
  'btn',
  'table__btn',
  'table__btn_delete',
  'js-btn-delete',
    ...
    case 'cancel':
      {
        ...
        pathLoad.setAttribute(
          'd',
          'M1.00008 6.04008C1.00008 8.82356 3.2566 11.0801 6.04008 11.0801C8.82356 11.0801 11.0801 8.82356 11.0801 6.04008C11.0801 3.2566 8.82356 1.00008 6.04008 1.00008C5.38922 1.00008 4.7672 1.12342 4.196 1.34812'
        );
...
function convertDate(initialDate) {
  const date =
    addZeroForDates(initialDate.getDate()) +
    '.' +
    addZeroForDates(initialDate.getMonth() + 1) +
    '.' +
    initialDate.getFullYear();
  const time =
    addZeroForDates(initialDate.getHours()) +
    ':' +
...
export function createTableRow(parent, obj) {
  //преобразование данных
  const fullName =
    obj.surname.trim() + ' ' + obj.name.trim() + ' ' + obj.lastName.trim();
  ...
  createElement(
    ...
    classesTableDate,
    createDateObj.date
  ).setAttribute('datetime', obj.createdAt);
  createElement(
    elementCreateDate,
    'span',
    classesTableTime,
  ...
  createElement(
    ...
    classesTableDate,
    updateDateObj.date
  ).setAttribute('datetime', obj.updatedAt);
  createElement(
    elementUpdateDate,
    'span',
    classesTableTime,
  ...
  const elementContactsList = createElement(
    elementContacts,
    'ul',
    classesTableContactsList
  );
  ...
  obj.contacts.forEach((contact) => {
    ...
    createContactLink(
      tooltip,
      contact.type,
      classesTableTooltipLink,
      contact.value
    );
  });

  const elementButtons = createElement(row, 'td', classesTableColumn);
  const elementButtonsContainer = createElement(
    elementButtons,
    'div',
    classesTableButtonsContainer
  );

  const buttonModify = createElement(
    elementButtonsContainer,
    'button',
    classesTableButtonModify
  );
